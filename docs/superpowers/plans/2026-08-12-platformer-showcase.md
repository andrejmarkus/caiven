...
platformer that exercises the full Caiven Lua API/stdlib as a
flagship showcase cart, per
`docs/superpowers/specs/2026-08-12-platformer-showcase-design.md`.
...
- Project lives at `projects/showcase/platformer/`; builds to
...
**Files:**
- Create: `projects/showcase/platformer/caiven.toml`
- Create: `projects/showcase/platformer/collision_types.json`
- Create: `projects/showcase/platformer/main.lua`
...
function _draw()
  clear_screen()
  draw_text("PLATFORMER - BOOT OK", 4, 60, 7)
end
```
...
target/debug/caiven-studio build projects/showcase/platformer --out /tmp/platformer.cav
cargo run -p caiven-machine -- /tmp/platformer.cav
```
Expected: window opens, sky-blue background, "PLATFORMER - BOOT OK" text
visible, no panics/Lua errors in terminal output. Close the window.

- [ ] **Step 5: Commit**

```bash
git add projects/showcase/platformer/caiven.toml \
        projects/showcase/platformer/collision_types.json \
        projects/showcase/platformer/main.lua
git commit -m "feat(showcase): scaffold platformer project"
```
...
**Files:**
- Create: `scripts/demo-carts/gen_platformer_assets.py`
- Create (generated by the script, then committed): `projects/showcase/platformer/sprites.hex`
- Create (generated by the script, then committed): `projects/showcase/platformer/sfx.hex`
- Create (generated by the script, then committed): `projects/showcase/platformer/music.hex`
- Modify: `projects/showcase/platformer/main.lua`
...
Run: `python3 scripts/demo-carts/gen_platformer_assets.py`
Expected: prints the output path; `sprites.hex`, `sfx.hex`, `music.hex` now
exist under `projects/showcase/platformer/`.
...
```bash
git add scripts/demo-carts/gen_platformer_assets.py \
        projects/showcase/platformer/sprites.hex \
        projects/showcase/platformer/sfx.hex \
        projects/showcase/platformer/music.hex \
        projects/showcase/platformer/main.lua
git commit -m "feat(showcase): generate platformer sprite/sfx/music assets"
```
...
**Files:**
- Modify: `projects/showcase/platformer/main.lua` (full rewrite of body,
...
```bash
git add projects/showcase/platformer/main.lua
git commit -m "feat(showcase): platformer world data, room painter, camera"
```
...
```bash
git add projects/showcase/platformer/main.lua
git commit -m "feat(showcase): platformer player run/jump/coyote/buffer physics"
```
...
```bash
git add projects/showcase/platformer/main.lua
git commit -m "feat(showcase): platformer wall slide and wall jump"
```
...
```bash
git add projects/showcase/platformer/main.lua
git commit -m "feat(showcase): platformer 8-directional dash"
```
...
```bash
git add projects/showcase/platformer/main.lua
git commit -m "feat(showcase): platformer hazards, death, and respawn"
```
...
```bash
git add projects/showcase/platformer/main.lua
git commit -m "feat(showcase): platformer berry collectibles"
```
...
```bash
git add projects/showcase/platformer/main.lua
git commit -m "feat(showcase): platformer flag, win screen, HUD, restart loop"
```
...
**Interfaces:**
- Produces: `platformer` entry in `EXAMPLES`, bumping its length from 5
  to 6.
...
`crates/caiven-studio/resources/examples/platformer.cav`, no errors.
...
Run: `cargo run -p caiven-machine -- crates/caiven-studio/resources/examples/platformer.cav`
...
git commit -m "feat(studio): add Platformer to the examples gallery"
```

If Step 5 required follow-up fixes to `projects/showcase/platformer/main.lua`,
rerun `scripts/demo-carts/build.sh` first so the committed `.cav` matches,
then commit those fixes separately:
```bash
git add projects/showcase/platformer/main.lua \
        crates/caiven-studio/resources/examples/platformer.cav
git commit -m "fix(showcase): tune platformer playtest feedback"
```
