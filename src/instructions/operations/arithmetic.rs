...
pub fn move_value(vm: &mut Vm, _input: &Input, _screen: &mut Screen) {
...
pub fn add_value(vm: &mut Vm, _input: &Input, _screen: &mut Screen) {
    let reg_index = vm.get_program()[vm.get_pc()] as usize;
    let value = vm.get_program()[vm.get_pc() + 1];
...
pub fn decrement_value(vm: &mut Vm, _input: &Input, _screen: &mut Screen) {
    let reg_index = vm.get_program()[vm.get_pc()] as usize;
