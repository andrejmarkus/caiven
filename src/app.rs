...
impl App {
    fn new() -> Self {
        ...
        let audio = match std::panic::catch_unwind(|| Audio::new(vm.get_sound_shared())) {
            Ok(audio) => Some(audio),
            Err(_) => {
...
pub fn run() {
    ...
    if let AppMode::Build(input_path, output_path) = mode {
        ...
        let source = std::fs::read_to_string(&input_path).unwrap_or_else(|e| {
            error!("Failed to read input file {}: {}", input_path.display(), e);
            std::process::exit(1);
        });
        ...
        write_rom(&output_path, &program).unwrap_or_else(|e| {
            error!("Failed to write ROM file {}: {}", output_path.display(), e);
            std::process::exit(1);
        });
