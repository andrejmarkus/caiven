...
use crate::section::{RomSection, SectionKind};

const MAGIC: &[u8; 6] = b"SPEAR2";
...
fn load_bytes(data: &[u8]) -> Result<Rom, RomError> {
    ...
    let header = RomHeader::from_bytes(header_buf);
...
/// Write a ROM with optional extra asset sections.
