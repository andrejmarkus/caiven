...
Builds with `--features automation` include `tauri-plugin-webdriver-automation`, driven via `tauri-wd` and the `tauri-automation` MCP server (`~/mcp-servers/mcp-tauri-automation`). Before calling any `tauri-automation` tool (`launch_app`, `click_element`, etc.), make sure both of these are running:
