...
fn c_error() -> Color { Color::new_rgb(200, 80, 80) }
...
enum OnlineState {
    Idle,
    Fetching,
    Loaded { carts: Vec<HubCart>, total: u64, page: u32 },
    Error(String),
}

enum HubMsg {
    CartList { carts: Vec<HubCart>, total: u64, page: u32 },
    RomReady(PathBuf),
    Err(String),
}
...
impl BrowserEditor {
    pub fn new() -> Self {
        ...
        let hub_url = std::env::var("FC_HUB_URL")
    ...
    fn fetch_page(&mut self, page: u32) {
        ...
        let url = format!("{}/api/carts?page={}&per_page={}", self.hub_url, page, PER_PAGE);
        std::thread::spawn(move || {
            let result = ureq::get(&url).call().map_err(|e| format!("CONNECTION FAILED: {e}"))
                .and_then(|resp| {
                    serde_json::from_reader::<_, HubCartList>(resp.into_reader())
                        .map_err(|e| format!("PARSE ERROR: {e}"))
                });
            match result {
                Ok(list) => { let _ = tx.send(HubMsg::CartList { carts: list.carts, total: list.total, page }); }
                Err(e) => { let _ = tx.send(HubMsg::Err(e)); }
            }
        });
    }
    ...
    fn download_rom(&mut self, id: String, title: String) {
        ...
        let safe: String = title.chars()
            ...
            .take(32)
            .collect();
        let tmp_path = std::env::temp_dir().join("fc-hub").join(format!("{}.rom", safe));
        std::thread::spawn(move || {
            ...
            let result = ureq::get(&url).call().map_err(|e| format!("CONNECTION FAILED: {e}"))
                .and_then(|resp| {
                    let mut buf = Vec::new();
                    resp.into_reader().read_to_end(&mut buf).map_err(|e| format!("READ ERROR: {e}"))?;
                    std::fs::write(&tmp_path, &buf).map_err(|e| format!("WRITE ERROR: {e}"))?;
                    Ok(tmp_path)
                });
            match result {
                Ok(path) => { let _ = tx.send(HubMsg::RomReady(path)); }
                Err(e) => { let _ = tx.send(HubMsg::Err(e)); }
            }
        });
    }
    ...
    fn render_local(&self, layer: &mut ScreenLayer, font: &Font) {
        if self.files.is_empty() {
            ...
            draw_text(font, layer, "PLACE .ROM FILES IN CURRENT DIR", Vec2::new(1, LIST_TOP + ROW_H), c_empty());
            draw_text(font, layer, "PRESS R TO RESCAN", Vec2::new(1, LIST_TOP + ROW_H * 3), c_hint());
            return;
        }
    ...
    fn render_online(&self, layer: &mut ScreenLayer, font: &Font) {
        if self.downloading {
            draw_text(font, layer, "DOWNLOADING...", Vec2::new(1, LIST_TOP), c_normal());
            return;
        }

        match &self.online_state {
            OnlineState::Idle => {
                draw_text(font, layer, "PRESS R TO BROWSE HUB", Vec2::new(1, LIST_TOP), c_empty());
                draw_text(font, layer, "SET FC-HUB-URL ENV VAR", Vec2::new(1, LIST_TOP + ROW_H), c_hint());
                draw_text(font, layer, "DEFAULT: LOCALHOST:8080", Vec2::new(1, LIST_TOP + ROW_H * 2), c_hint());
            }
            OnlineState::Fetching => {
                draw_text(font, layer, "LOADING...", Vec2::new(1, LIST_TOP), c_normal());
            }
            OnlineState::Error(e) => {
                ...
                draw_text(font, layer, "PRESS R TO RETRY", Vec2::new(1, LIST_TOP + ROW_H * 3), c_hint());
            }
            OnlineState::Loaded { carts, total, page } => {
                ...
                for (vis_idx, cart_idx) in (self.online_scroll..visible_end).enumerate() {
                    ...
                    draw_text(font, layer, &title, Vec2::new(2, y + 1), if is_sel { c_selected() } else { c_normal() });
                }
                ...
                if let Some(cart) = carts.get(self.online_selected) {
                    ...
                    let hint = format!("{}  {}DL  PG {} OF {}", author, dl, page + 1, total_pages.max(1));
...
impl Editor for BrowserEditor {
    fn render(&self, layer: &mut ScreenLayer, _vm: &Vm, font: &Font, _cursor: (u32, u32)) {
        ...
        let online_col = if self.tab == BrowserTab::Online { c_selected() } else { c_hint() };
