...
export const api = {
  ...
  createCart: (rom: File, meta: { title: string; author: string; description: string; tags: string[] }) => {
  ...
  createVersion: (id: string, rom: File, changelog: string) => {
    const form = new FormData();
    form.set('rom', rom);
    form.set('meta', JSON.stringify({ changelog }));
    return request<Cart>(`/carts/${id}/versions`, { method: 'POST', body: form });
  },
  romUrl: (id: string, version?: number) => `${BASE}/carts/${id}/rom${qs({ version })}`,
